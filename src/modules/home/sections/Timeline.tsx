  ...
  <section id="progresso" className="section-padding max-w-6xl mx-auto">
      ...
      <div className="block lg:hidden -mx-4 xs:-mx-6 pb-4">
        <div className="flex overflow-x-auto gap-4 px-4 xs:px-6 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-brand-green/40 scrollbar-track-transparent">
          {phases.map(p => (
            <div key={p.title} className="snap-start min-w-[240px] w-[260px] card-responsive card-hover-responsive bg-white border border-brand-green/15 flex flex-col gap-2 shadow-soft">
              <div className="flex items-start justify-between gap-3">
                <h3 className="text-fluid-sm font-semibold leading-snug">{p.title}</h3>
                <span className="text-fluid-sm uppercase tracking-wide px-2 py-1 rounded-full bg-brand-green/10 text-brand-green whitespace-nowrap">{p.status}</span>
              </div>
              <p className="text-fluid-sm text-brand-dark/70 leading-relaxed flex-1">{p.desc}</p>
              <p className="text-fluid-sm text-brand-dark/50">{p.date}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="hidden lg:grid grid-cols-5 gap-6">
        {phases.map(p => (
          <div key={p.title} className="card-responsive card-hover-responsive bg-white border border-brand-green/15 flex flex-col gap-2 shadow-soft">
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-fluid-base font-semibold leading-snug">{p.title}</h3>
              <span className="text-fluid-sm uppercase tracking-wide px-2 py-1 rounded-full bg-brand-green/10 text-brand-green whitespace-nowrap">{p.status}</span>
            </div>
            <p className="text-fluid-sm text-brand-dark/70 leading-relaxed flex-1">{p.desc}</p>
            <p className="text-fluid-sm text-brand-dark/50">{p.date}</p>
      ...
      <p className="text-fluid-sm text-brand-dark/50 mt-6">Linha do tempo ilustrativa. Datas sujeitas a alterações conforme cronograma executivo.</p>
    </section>
  );
}
